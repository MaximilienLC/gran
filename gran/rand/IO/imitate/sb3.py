...
from gran.rand.IO.imitate.base import BaseImitateIO, BaseTarget
from gran.util.gym_state_control import get_task_name
...
class IO(BaseImitateIO):
    ...
    def load_target(self):
        ...
        dir_path = (
            ...
            + self.model_name
            + "/"
            + get_task_name(cfg.task)
            + "_1/"
        )

        return Target(
            self.model.load(
                dir_path + get_task_name(cfg.task) + ".zip",
                custom_objects=dict,
                device="cpu",
            )
        )
