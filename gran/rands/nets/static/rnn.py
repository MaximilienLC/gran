...
from gran.rands.nets.static.base import StaticNetBase
...
class Net(StaticNetBase):
    def __init__(self, dimensions, recurrent=True, batch_size=1):
        ...
        # recurrence on hidden_n layer (output layer if no hidden layer)
        ...
        for i, _ in enumerate(dimensions[:-1]):
            ...
            if (
                len(dimensions) == 2 or i + 3 == len(dimensions)
            ) and recurrent == True:
