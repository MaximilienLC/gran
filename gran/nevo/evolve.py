...
def evolve():
    ...
    assert not (cfg.alg == "es" and cfg.ecosystem.gen_transfer != "none")
    assert not (cfg.alg == "es" and cfg.ecosystem.pop_merge == "yes")
    ...
    if cfg.wandb_logging:
        ...
        assert "WANDB_API_KEY" in os.environ, "Logging with wandb requires"
    ...
    ecosys = getattr(import_module(cfg.ecosystem.space_path), "Env")()

    agents = None
    agents_batch = []
    agents_batch_size = cfg.pop_size // size

    if cfg.alg == "ga":
        # [MPI buffer size, pair position, sending, seed]
        exchange_and_mutate_info = None
        exchange_and_mutate_info_batch = np.empty(
            (bots_batch_size, ecosys.num_pops, 4), dtype=np.uint32
        )

    # [fitnesses, # env steps]
    fitnesses_and_num_env_steps_batch = np.empty(
        (bots_batch_size, ecosys.num_pops, 2), dtype=np.float32
    )

    # [fitnesses, # env steps, pickled agent sizes]
    generation_results = None
    generation_results_batch = np.empty(
        (bots_batch_size, ecosys.num_pops, 3), dtype=np.float32
    )

    if rank == 0:
        if cfg.alg == "ga":
            exchange_and_mutate_info = np.empty(
                (cfg.pop_size, ecosys.num_pops, 4), dtype=np.uint32
            )

        fitnesses = np.empty((cfg.pop_size, ecosys.num_pops), dtype=np.float32)

        generation_results = np.empty(
            (cfg.pop_size, ecosys.num_pops, 3), dtype=np.float32
        )
    ...
    if cfg.prev_num_gens > 0:
        ...
        to secondary procs.
        """
        agents_batch = comm.scatter(bots, root=0)

    """
    Setting up of GPU-evaluation specific MPI communication.
    """
    if ecosys.evaluates_on_gpu:
        ...
        ith_gpu_agents_batch_size = cfg.pop_size // num_gpus
        ...
        ith_gpu_fitnesses_and_num_env_steps_batch = np.empty(
            (ith_gpu_agents_batch_size, ecosys.num_pops, 2), dtype=np.float32
        )
    ...
    for curr_gen in range(cfg.prev_num_gens, cfg.prev_num_gens + cfg.num_gens):
        ...
        if rank == 0:
            ...
            if cfg.alg == "ga":
                new_seeds = generate_new_seeds(curr_gen, ecosys.num_pops)

                if curr_gen != 0:
                    for j in range(ecosys.num_pops):
            ...
            if cfg.alg == "es":
                ...
                for i in range(bots_batch_size):
                    for j in range(ecosys.num_pops):
                        ...
                        if exchange_and_mutate_info_batch[i, j, 2] == 1:
                            tag = int(
                                cfg.pop_size * j + agents_batch_size * rank + i
                            )
                ...
                for i, agent in enumerate(received_agents):
                    if agent is not None:
                        agents_batch[i // ecosys.num_pops][
                            i % ecosys.num_pops
                        ] = agent
        ...
        for i in range(bots_batch_size):
            ...
            ecosys.mutate_agents(seeds, curr_gen)

            """
            CPU Evaluation.
            """
            if ecosys.evaluates_on_gpu == False:
                fitnesses_and_num_env_steps_batch[i] = ecosys.run_agents(
                    curr_gen
                )

                if curr_gen == 0:
                    agents_batch.append(copy.deepcopy(ecosys.bots))
        ...
        if ecosys.evaluates_on_gpu:
            ...
            if is_gpu_queuing_cpu_proc:
                ith_gpu_fitnesses_and_num_env_steps_batch = ecosys.run_agents(
            ...
            if cfg.ecosystem.gen_transfer != "none":
        ...
        if rank == 0:
            ...
            if cfg.ecosystem.pop_merge == "yes":
        ...
        State saving
        """
        if curr_gen + 1 in ecosys.io.save_points or curr_gen == 0:
            batched_agents = comm.gather(bots_batch, root=0)
            ...
            if rank == 0:
                ...
                ecosys.io.save_state(
                    [bots, fitnesses, total_num_env_steps], curr_gen + 1
                )
