...
class BaseImitationSpace(BaseSpace):
    """
    ...
    def verify_assertions(self) -> None:
        ...
        assert hasattr(self, "hide_env_score"), "Attribute 'hide_env_score' "
        ...
        assert isinstance(cfg.ecosystem.pop_merge, bool)
    ...
    def reset(self, curr_gen: int) -> np.ndarray:
        ...
        if "env" in cfg.ecosystem.gen_transfer:
            if curr_gen == 0:
                self.curr_actor_data_holder.saved_env_seed = new_seed

            obs = self.reset_env_state(
                self.curr_env,
            ...
            obs = self.reset_env_state(self.curr_env, new_seed)
    ...
    def run_agents(self, curr_gen: int) -> float:
        ...
        """
        if curr_gen == 0:
            assert hasattr(self, "envs"), "Attribute 'envs' required: the "
            "one or two environments to run the agents on."

            assert hasattr(self, "imitation_target"), "Attribute "
            "'env' required: the target behaviour/agent to imitate."
        ...
        for match in [0, 1]:
            ...
            hidden_score_obs = self.hide_score(obs, cfg.ecosystem.task)
            num_obs = 0
            ...
            while not done:
                ...
                obs, rew, done = self.run_env_step(self.curr_env, output)
                hidden_score_obs = self.hide_score(obs, cfg.ecosystem.task)
            ...
            p_imitation_target /= cfg.ecosystem.run_num_steps
