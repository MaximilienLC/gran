...
class BaseSpace(ABC):
    ...
    def verify_assertions(self) -> None:
        ...
        assert config.task in self.valid_tasks

        assert (
            isinstance(config.seed, int) and config.seed >= 0
        ) or config.seed == "scheduled"

        assert (
            isinstance(config.ecosystem.run_num_steps, int)
            and config.ecosystem.run_num_steps >= 0
        )

        assert config.ecosystem.transfer in [
            "none",
            "fit",
            "env+fit",
            "mem+env+fit",
    ...
    def initialize_agents(self) -> None:
        ...
        for pop_idx in range(self.num_pops):
            self.agents.append(
                getattr(import_module(config.ecosystem.agent_path), "Agent")(
                    pop_idx
                )
            )
