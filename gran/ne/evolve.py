...
from gran.ne.util.misc import generate_new_seeds
...
def evolve():
    ...
    if config.ecosystem.algorithm == "es":
        assert not config.ecosystem.transfer
        assert not config.ecosystem.merge
    ...
    space = getattr(import_module(config.ecosystem.space_path), "Space")()
    ...
    if config.ecosystem.algorithm == "ga":
        ...
        exchange_and_mutate_info_batch = np.empty(
            (bots_batch_size, space.num_pops, 4), dtype=np.uint32
        )

    # [fitnesses, # env steps]
    fitnesses_and_num_env_steps_batch = np.empty(
        (bots_batch_size, space.num_pops, 2), dtype=np.float32
    )

    # [fitnesses, # env steps, pickled agent sizes]
    generation_results = None
    generation_results_batch = np.empty(
        (bots_batch_size, space.num_pops, 3), dtype=np.float32
    )
    ...
    for curr_gen in range(
        ...
        for i in range(bots_batch_size):
            ...
            if space.evaluates_on_gpu == False:
                ...
                if curr_gen == 0:
                    agents_batch.append(copy.deepcopy(space.bots))
        ...
        if space.evaluates_on_gpu:
            ...
            if config.ecosystem.transfer != "none":
