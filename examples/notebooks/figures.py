    ...
    for nn in nns:
        ...
        for pop in pops:
            ...
            for gen in reversed(gens):
                ...
                with open(
                    os.path.join(path, exp, nn, pop, gen, "state.pkl"), "rb"
                ) as f:
                    state = pickle.load(f)
                    _, _, num_steps = state
                f.close()
            ...
            scores = np.stack(scores)  # (gens, pop // 2, num_tests)
            ...
            exp_dict[task][seed][transfer][net].append(
                [pop, gens, steps, best_individual, mean_individual]
            )
